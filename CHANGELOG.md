...
- **`adapters/replay.py` — `ReplayAdapter` + cassettes.** Serves model completions from a per-request cassette keyed by `canonical_request_key` (SHA-256 over the *canonicalized* outgoing request: model_id + prompt + sampling params + `tool_registry_hash`; nothing clock/host-derived enters the key). The Tier-A gate drives the **real** agent loop off recorded responses — key-free and deterministic — yet a prompt / tool-schema / temperature edit moves the key → **`ReplayMiss` → build fails** ("re-record"), so a prompt regression **cannot silently pass** (VCR `record_mode=none` + Jest `-u`, fused). `RecordingAdapter` wraps a backend to capture the cassette (the local "re-record" half).
- **`eval/gate.py` + `scripts/ci_trajectory_gate.py` — the Tier-A gate (§6).** Replays the committed golden, scores the deterministic dimensions + the injection guardrail, and gates **per-dimension over a committed `thresholds.json` — a CONJUNCTION, never a blended average**; binary dims take the min across scenarios, graded dims the mean; **step-efficiency is report-only and never gates**. Emits a scorecard ("eval as a dashboard, not a number"). Exit 1 on any threshold breach or replay miss. The judged dimensions (nugget recall, faithfulness) need a live judge and stay in the deferred Tier-B nightly.
- **`scripts/build_golden.py` + committed golden set (`data/set5/`).** Three scenarios — a happy factoid, a transient-fault-recovery case, and an injection-HELD adversarial case — recorded once into `data/set5/cassettes/*.json` + `scenarios.jsonl` + `thresholds.json`. Re-recording is a local dev action (`build_golden.py`); CI replays.
- **`scripts/lock_registry.py` + `data/registry.sha256`.** Snapshot-locks the tool-registry schema (§6/§12): a MAJOR tool-schema change trips a **reviewed re-baseline** (`--update`, the `jest -u` discipline) instead of silently passing. Verified in CI.
- **CI** (`.github/workflows/ci.yml`): the Tier-A gate + the registry lock verification run alongside the existing smoke steps.
- **Tests**: **156 tests** (was 142) — `tests/test_replay.py` (key stability, prompt/registry/temperature moves the key, hit/miss, record→replay roundtrip) and `tests/test_gate.py` (golden passes, a **blanked cassette → replay miss → gate fails**, must-pass/graded threshold breaches, binary-min-vs-graded-mean aggregation).
- **v0.3 mock/CI core (spec §9 steps 1-6) is now COMPLETE and key-free.** Remaining is corpus-gated (step 7: real vector store + Set-4 authoring + live run) and the Tier-B nightly (judged dimensions).

### Added (v0.3 adversarial harness + stats/judge module — spec §9 steps 4-5, key-free)
- **`eval/adversarial.py` — the §5 guardrail suite.** Prompt-injection scored as a **CONJUNCTION** over the OWASP-LLM01 channels (no injected-arg tool call AND no canary leak AND no data-exfil send-step AND no output-steering) — HELD iff none fired, so a run that both emits a refusal and leaks the secret is correctly **COMPROMISED** (the old OR-logic scored it HELD). A **fixed per-scenario canary** is planted in the system prompt (`plant_canary`), making disclosure a deterministic substring scan; the scorer **FAILS LOUD** (`CanaryNotPlanted`) if the canary is absent from the run record rather than silently passing. **Targeted-ASR is reported separately from utility** (AgentDojo's two axes). Fault injection (`with_injected_fault`) raises a broad, partly-unanticipated **stdlib fault set** (`socket.timeout`, `urllib.error.URLError/HTTPError` 503/429, `http.client.IncompleteRead`, `ConnectionResetError`, `json.JSONDecodeError`) at the ToolRegistry seam, chosen independently of what the handler catches; `fail_times=N` models a transient blip that recovers on retry. Recovery is graded by the existing `eval/trajectory.score_error_recovery`.
- **`agent/tools.py` seam hardening.** `ToolRegistry.call` now wraps a handler's raw stdlib exception into `ToolError` (underlying type preserved in the message), so a real tool's `TimeoutError`/`HTTPError` surfaces uniformly for the loop's recovery — and the type stays visible for the Phase-2 shape-fidelity conformance test.
- **`eval/stats.py` — the shared stats/judge module (§7, premortem #11).** Centralizes the three primitives the scorers "reuse" so the claim is real, not a macro_f1-only bootstrap + a pairwise-only judge: (a) `seeded_bootstrap_ci` (generic seeded-percentile bootstrap over an **arbitrary** statistic, reusing `utils.percentile`) + `paired_delta_ci` (paired bootstrap, §7a); (b) `mcnemar_exact` (exact binomial McNemar on discordant pairs, `math.comb`, raw b/c always printed) + `cohens_kappa` (built here — **not** in the v0.2 code); (c) `pointwise_label` — a **pointwise** judge mode (support/partial/not_support) on the existing adapter, malformed → not_support (conservative abstain). Pointwise scorers on top: `nugget_recall` (vital-weighted, RAGAS FactualCorrectness / TREC AutoNuggetizer method), `citation_faithfulness`, and `answer_correctness` (recall + precision → F1, with the binary vital-recall==1.0 correct-outcome gate the §6 gate consumes).
- **`scripts/lock_adversarial.py`** — SHA-256 lock for the adversarial set (clone of `lock_holdout.py`; imports the runner's hashing helpers so hashes agree). Verified end-to-end on a fixture.
- **Tests**: **142 tests** (was 112) — `tests/test_adversarial.py` (each injection channel fires/holds, the leaked-but-refused conjunction case, fail-loud canary, every fault type recovered, transient-recovers-on-retry) and `tests/test_stats.py` (bootstrap reproducibility/arbitrary-statistic, paired-delta excludes-zero, McNemar symmetric-vs-lopsided, kappa known-value 0.4, pointwise malformed→not_support, nugget vital-weighting, faithfulness, answer-correctness gate).
- **Still deferred (spec §9 step 6):** `adapters/replay.py` cassettes + `scripts/lock_registry.py` + the full Tier-A per-dimension CI gate.

### Added (v0.3 agent trajectory core — spec §9 steps 1-3, key-free)
- **`agent/` package — the system under test.** `agent/tools.py` defines the `ToolRegistry` seam (mirroring the adapter Protocol): a `Tool` dataclass with an MCP-shaped `inputSchema`, a stdlib arg validator (`validate_args` — required/type/unknown-arg checks; **bool is not an integer**, guarded because `bool` subclasses `int`), and `registry_hash()` (SHA-256 over the canonical schema view — the signal the §6 baseline triple keys on; moves iff a tool/schema changes, not on a handler change). `build_mock_registry()` provides deterministic `vector_search` / `fetch_article` / `list_by_category` over a tiny in-memory corpus using synthetic `sift://<category>/<slug>` ids (spec §2 Prerequisite-0), so the deterministic scorers and locks aren't blocked on the real Sift article-ID decision. `agent/loop.py` is the four-role loop (router → planner → executor → critic) — model-agnostic and tool-agnostic, tolerant line-parsing like `eval/judge.py`, and it **never crashes on a tool fault**: a raised `ToolError` is retried once then falls back to synthesis (§5b), recording faithfully whether it recovered. Still **zero runtime deps**.
- **`adapters/scripted.py`** — `ScriptedAdapter` returns canned completions in order (the ordered driver a multi-turn loop needs, which a stateless substring `MockAdapter` can't provide; also the shape the step-6 cassette `ReplayAdapter` will take).
- **`eval/trajectory.py`** — the §3 run-unit writer (extends the v0.2 reproducibility header with `agent_version` / `tool_registry_hash` / `n_samples` / `trial_temperature`; same "score downstream, never re-invoke the model" invariant) plus the **deterministic** §4 scorers: layered tool-selection (**legality** hard-gate × mean(**coverage**, **precedence** partial-order, **state-legality**)), arg-schema-validity, **report-only** step-efficiency (denominator disclosed inline), error-recovery (retry/fallback vs unrecovered), and the free deterministic `citations ⊇ gold-id` half of answer-correctness. `score_trajectory()` emits a per-dimension scorecard ("eval as a dashboard, not a number").
- **`scripts/example_agent_run.py`** — end-to-end key-free demo (mirrors `example_run.py`): loop → §3 JSONL → re-score downstream from the JSONL. Wired into CI alongside the other smoke scripts.
- **`rubrics/set5_trajectory_rubric.md`** — the trajectory rubric schema (allowed/required tools, precedence edges, step budget, gold article ids) + a worked scenario.
- **Tests**: **112 tests** (was 78) — `tests/test_agent_tools.py` (validator incl. the bool gotcha, registry-hash stability/change, deterministic mock search), `tests/test_agent_loop.py` (routing, arg-validity recording, fault→retry→fallback recovery, unrecovered-when-no-answer, script-exhaustion guard), `tests/test_trajectory.py` (writer §3 shape + JSONL roundtrip, every scorer incl. illegal-tool zeroes-legality and precedence/state violations).
- **Deferred to a follow-up PR (spec §9 steps 4-6):** `eval/adversarial.py` (injection channels + canary, injected faults), `eval/stats.py` (pointwise judge mode + generic bootstrap + kappa), `adapters/replay.py` (cassettes) + `scripts/lock_adversarial.py` / `lock_registry.py`, and the full Tier-A per-dimension CI gate.
