...
**Audience:** ML engineers and researchers who want to evaluate the eval. The 2-minute landing summary lives on the leaderboard front page; this page is the deep link.
**Last updated:** 2026-05-26
**Source:** [spec v0.2](../eval-harness-spec.md) · [decision log](../CHANGELOG.md) · [repo](https://github.com/kristenmartino/eval-harness)
...
Macro-F1 is reported alongside accuracy because Sift's category distribution is imbalanced (the §8 pre-flight check drops categories with fewer than 20 articles rather than upsampling — upsampling biases macro-F1 upward). Label noise estimated on a 100-article subsample of Set 1; the noise rate is reported alongside accuracy so the headline number is interpretable.
...
**API models** — direct token-priced cost at posted rates as of run date. Pricing pinned in [`scripts/judge_cost_budget.py`](../scripts/judge_cost_budget.py) so any reader can re-cost.
...
Prompt content is hashed and stored in `prompts/` with the version embedded in each task module. The Ollama version is documented separately in [`CHANGELOG.md`](../CHANGELOG.md).
...
*Decision log and changelog: see [CHANGELOG.md](../CHANGELOG.md). Full spec: [eval-harness-spec.md](../eval-harness-spec.md).*
