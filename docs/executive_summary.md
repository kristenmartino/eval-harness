...
- **Harness infrastructure** end-to-end: adapter Protocol (Ollama + Anthropic + OpenAI), task modules (categorization + summarization), runner with reproducibility headers + resumability + enforced held-out gate + CLI, Bradley-Terry pairwise ranking, 47 tests passing (math, judge parsing, runner reproducibility + held-out gate), GitHub Actions CI.
